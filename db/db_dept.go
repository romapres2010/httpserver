...
func (s *Service) getDept(ctx context.Context, tx *sql.Tx, out *model.Dept) (exists bool, myerr error) {
	...
	if out != nil {
		...
		return sql.Get(reqID, tx, s.SQLStms["GetDept"], out, out.Deptno)
	}
	return false, myerror.New("4400", "Incorrect call: reqID", reqID).PrintfInfo()
}

// getDeptsPK return a PK for all Dept
func (s *Service) getDeptsPK(ctx context.Context, tx *sql.Tx, out *model.DeptPKs) (myerr error) {
	reqID := myctx.FromContextRequestID(ctx) // RequestID передается через context
	mylog.PrintfDebugMsg("START: reqID", reqID)

	if out != nil {
		return sql.Select(reqID, tx, s.SQLStms["GetDeptsPK"], out)
	}
	return myerror.New("4400", "Incorrect call: reqID", reqID).PrintfInfo()
}

// createDept create new Dept
func (s *Service) createDept(ctx context.Context, tx *sql.Tx, in *model.Dept, out *model.Dept) (myerr error) {
	reqID := myctx.FromContextRequestID(ctx) // RequestID передается через context
	mylog.PrintfDebugMsg("START: reqID", reqID)

	if in != nil && tx != nil {
		// Создаем объект в рамках транзации
		if myerr = sql.Create(reqID, tx, s.SQLStms["GetDept"], s.SQLStms["CreateDept"], in, out, in.Deptno); myerr != nil {
			return myerr
		}
...
func (s *Service) updateDept(ctx context.Context, tx *sql.Tx, in *model.Dept, out *model.Dept) (myerr error) {
	reqID := myctx.FromContextRequestID(ctx) // RequestID передается через context
	mylog.PrintfDebugMsg("START: reqID", reqID)

	if in != nil && tx != nil {
		// Обновляем объект в рамках транзации
		if myerr = sql.Update(reqID, tx, s.SQLStms["GetDept"], s.SQLStms["UpdateDept"], in, out, in.Deptno); myerr != nil {
			return myerr
		}
