...
func InitDbTest() *Service {
	...
	pqServiceCfg := Config{
		PqHost:            "130.61.117.149",
		PqPort:            "5432",
		PqDbname:          "test_database",
		PqSslMode:         "disable",
		PqUser:            "test_user",
		PqPass:            "qwerty",
		PqConnMaxLifetime: 1000,
		PqMaxOpenConns:    16,
		PqMaxIdleConns:    8,
		PqDriverName:      "pgx",
	}
...
func BenchmarkPgDb_GetDept(b *testing.B) {
	...
	deptsPK := model.GetDeptsPK() // Извлечем из pool новую структуру
...
func BenchmarkPgDb_CreateDept(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() { // The loop body is executed b.N times total across all goroutines
			...
			err := tests.p.pqDb.Get(&depnoPK, "select nextval('dept_deptno_seq') deptno")
			if err != nil {
			...
			for i := range deptNew.Emps {
				...
				err := tests.p.pqDb.Get(&empnoPK, "select nextval('dept_deptno_seq') empno")
				if err != nil {
					b.Errorf("\n PgDb.CreateDept() - error tests.p.Db.Get(&depnoPK, select nextval('dept_deptno_seq')) %v", fmt.Sprintf("%+v", err))
					return
				}
